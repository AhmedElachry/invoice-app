...
function InvoiceInfo() {
  ...
  const onMakePaidClick = () => {
    dispatch(
      invoicesSlice.actions.updateInvoiceStatus({
        id: invoiceId,
        status: "paid",
      })
    );
    dispatch(invoicesSlice.actions.getOneInvoice({ id: invoiceId }));
  };
  ...
  const onDelete = () => {
    dispatch(invoicesSlice.actions.deleteInvoice({ id: invoiceId }));
  };
  ...
  const invoice = useSelector((state) => state.invoices.selectedInvoiceById);
  ...
  return (
    <div>
      {invoice ? (
        ...
        >
          <div className="">
            ...
            <div className="  mt-8 rounded-lg w-full flex items-center  justify-between   px-6 py-6 bg-white dark:bg-[#1e2139]">
              <div className=" hidden sm:flex  space-x-2 md:justify-start md:w-auto  items-center">
            ...
            <div className=" mt-4 rounded-lg w-full  px-6 py-6 bg-white dark:bg-[#1e2139]">
              ...
              <div className=" sm:hidden mt-10 bg-[#f9fafe] dark:bg-[#252945] rounded-lg rounded-b-none space-y-4  p-10">
                {invoice?.items?.map((item) => (
                  <div
                    key={item?.name}
                    className=" justify-between text-lg dark:text-white flex"
                  >
                    <h1>{item?.name}</h1>
                    <h1>£{item?.total}</h1>
                  </div>
        ...
        <div className="mx-auto mt-[200px] w-fit dark:text-white">
      ...
      <AnimatePresence>
        {isEditOpen && (
          <CreateInvoice
            invoice={invoice}
            type="edit"
            setIsCreateInvoiceOpen={setIsEditOpen}
          />
        )}
      </AnimatePresence>
    </div>
